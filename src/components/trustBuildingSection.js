...
export default function TrustBuildingSection() {
  ...
  const [windowWidth, setWindowWidth] = useState(1024); // ✅ ancho por defecto para SSR seguro

  // ✅ Manejamos el tamaño de pantalla de forma segura
  useEffect(() => {
  ...
  const guarantees = [
    {
      ...
      description:
        "Durante todo el proyecto, el cliente sabe en qué etapa estamos y qué se está trabajando.",
    ...
    },
    {
      ...
      description:
        "El precio se define antes de comenzar y se mantiene hasta la entrega final.",
    ...
    },
    {
      ...
      description:
        "Se establece un plazo estimado realista según la complejidad del proyecto.",
    ...
    },
    {
      ...
      description:
        "Durante las primeras semanas después de la entrega, se ofrece ayuda para resolver dudas o pequeños ajustes.",
      highlight: "Incluido",
      color: "#e2c674",
    },
    {
      ...
      description:
        "Cada web se entrega lista para cargar rápido y adaptada a cualquier dispositivo.",
    ...
    },
    {
      ...
      description:
        "Cada cliente recibe atención directa, sin intermediarios, con enfoque en sus objetivos reales.",
  ...
  // ✅ Protegemos el uso de window.open()
  const handleMaintenanceClick = () => {
    const message =
      "¡Hola Codegza! %0A%0AEstoy interesado/a en el *Plan de Mantenimiento Anual* que vi en la web.%0A%0AMe gustaría consultar:%0A• Precio del plan anual%0A• Qué tipos de actualizaciones incluye%0A• Cómo funciona el servicio mensual%0A• Condiciones y disponibilidad%0A%0A¡Gracias!";
              ...
              >
                <div className="px-3 sm:px-4 md:px-5 py-2 sm:py-2.5 md:py-3 bg-gradient-to-br from-[#2a2416] to-[#1a1510] border border-[#cfae01]/30 rounded-full hover:border-[#cfae01] hover:shadow-[0_0_20px_rgba(207,174,1,0.4)] transition-all duration-300 cursor-pointer">
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    ...
                    <span className="text-xs sm:text-sm md:text-base text-white font-medium group-hover:text-[#cfae01] transition-colors">
                      {tech.name}
                    </span>
                  </div>
                </div>
              ...
              {optionalServices[0].features.map((feature, i) => (
                <div key={i} className="flex items-start gap-2 sm:gap-3">
                  <div className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 rounded-full bg-[#cfae01]/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                    <svg
                      className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-4 md:h-4 text-[#cfae01]"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
